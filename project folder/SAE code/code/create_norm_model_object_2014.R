...
clust_medfev_key <- left_join(medfev_clust_Y, key_14)
...
# Create SPDE
# medfever.spde <- inla.spde2.pcmatern(mesh = medfever.mesh, alpha = 2,
#                                      prior.range = c(.05, 0.1), # P(range < 1) = 0.9
...
cov_IM_list_Y[["precip_2014-06"]]$v <- log(1 + cov_IM_list_Y[["precip_2014-06"]]$v)
cov_IM_list_Y[["precip_2014-07"]]$v <- log(1 + cov_IM_list_Y[["precip_2014-07"]]$v)
cov_IM_list_Y[["precip_2014-08"]]$v <- log(1 + cov_IM_list_Y[["precip_2014-08"]]$v)
cov_IM_list_Y[["precip_2014-09"]]$v <- log(1 + cov_IM_list_Y[["precip_2014-09"]]$v)
cov_IM_list_Y[["precip_2014-10"]]$v <- log(1 + cov_IM_list_Y[["precip_2014-10"]]$v)
cov_IM_list_Y[["precip_2014-11"]]$v <- log(1 + cov_IM_list_Y[["precip_2014-11"]]$v)


cov_IM_list_Y[["air.temp_2014-06"]]$v <- log(1 + cov_IM_list_Y[["air.temp_2014-06"]]$v)
cov_IM_list_Y[["air.temp_2014-07"]]$v <- log(1 + cov_IM_list_Y[["air.temp_2014-07"]]$v)
cov_IM_list_Y[["air.temp_2014-08"]]$v <- log(1 + cov_IM_list_Y[["air.temp_2014-08"]]$v)
cov_IM_list_Y[["air.temp_2014-09"]]$v <- log(1 + cov_IM_list_Y[["air.temp_2014-09"]]$v)
cov_IM_list_Y[["air.temp_2014-10"]]$v <- log(1 + cov_IM_list_Y[["air.temp_2014-10"]]$v)
cov_IM_list_Y[["air.temp_2014-11"]]$v <- log(1 + cov_IM_list_Y[["air.temp_2014-11"]]$v)


cov_IM_list_Y$log_pop <- cov_IM_list_Y$pop_2014
...
# clust_medfev_binom_key_fitting$NDVI <- 0
# clust_medfev_binom_key_fitting$NDVI_lag1 <- 0


covs_df_D1 <- covs_df_w_dates[, grepl("2014-06", names(covs_df_w_dates))]

clust_medfev_binom_key_fitting[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-09")), grep("lag3", names(clust_medfev_binom_key_fitting))] <- covs_df_D1[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-09")),]


covs_df_D2 <- covs_df_w_dates[, grepl("2014-07", names(covs_df_w_dates))]

clust_medfev_binom_key_fitting[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-10")), grep("lag3", names(clust_medfev_binom_key_fitting))] <- covs_df_D2[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-10")),]
clust_medfev_binom_key_fitting[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-09")), grep("lag2", names(clust_medfev_binom_key_fitting))] <- covs_df_D2[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-09")),]


covs_df_D3 <- covs_df_w_dates[, grepl("2014-08", names(covs_df_w_dates))]

clust_medfev_binom_key_fitting[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-11")), grep("lag3", names(clust_medfev_binom_key_fitting))] <- covs_df_D3[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-11")),]
clust_medfev_binom_key_fitting[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-10")), grep("lag2", names(clust_medfev_binom_key_fitting))] <- covs_df_D3[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-10")),]
clust_medfev_binom_key_fitting[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-09")), grep("lag1", names(clust_medfev_binom_key_fitting))] <- covs_df_D3[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-09")),]


covs_df_D4 <- covs_df_w_dates[, grepl("2014-09", names(covs_df_w_dates))]

clust_medfev_binom_key_fitting[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-11")), grep("lag2", names(clust_medfev_binom_key_fitting))] <- covs_df_D4[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-11")),]
clust_medfev_binom_key_fitting[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-10")), grep("lag1", names(clust_medfev_binom_key_fitting))] <- covs_df_D4[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-10")),]
clust_medfev_binom_key_fitting[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-09")), (grep("lag1", names(clust_medfev_binom_key_fitting))-1)] <- covs_df_D4[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-09")),]


covs_df_D5 <- covs_df_w_dates[, grepl("2014-10", names(covs_df_w_dates))]

clust_medfev_binom_key_fitting[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-11")), grep("lag1", names(clust_medfev_binom_key_fitting))] <- covs_df_D5[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-11")),]
clust_medfev_binom_key_fitting[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-10")), (grep("lag1", names(clust_medfev_binom_key_fitting))-1)] <- covs_df_D5[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-10")),]


covs_df_D6 <- covs_df_w_dates[, grepl("2014-11", names(covs_df_w_dates))]

clust_medfev_binom_key_fitting[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-11")), (grep("lag1", names(clust_medfev_binom_key_fitting))-1)] <- covs_df_D6[which(clust_medfev_binom_key_fitting$interview_date == as.yearmon("2014-11")),]
...
norm_list <- apply(clust_medfev_binom_key_fitting[,40:54], 2, get_norm)

clust_medfev_binom_key_fitting[,40:54] <- do.call(cbind, lapply(norm_list, function(X) X[[1]]))
...
cov_df <- cbind(b0 = 1, clust_medfev_binom_key_fitting[,40:434])
cov_df <- cbind(cov_df, clust_medfev_binom_key_fitting[,c("ADM1NAME", "NOMDEP")])

names(cov_df)[c(3,397:398)] <- c("pop", "Region", "District")
...
cov_IM_list_Y_preds <- cov_IM_list_Y[c("precip_2014-07", "precip_2014-08",
                                       "precip_2014-09", "precip_2014-10",
                                       "air.temp_2014-07", "air.temp_2014-08",
                                       "air.temp_2014-09", "air.temp_2014-10",
                                       # "NDVI_2014-09", "NDVI_2014-10",
                                       "public_tt", "pop_2014", "build_up",
                                       "log_pop", "log_build_up")]
...
scaling_factors_new <- scaling_factors[c(11:8,15:12, 1:7),]
...
names(covs.pred) <- c("precip_lag3", "precip_lag2", "precip_lag1",
                      "precip", "air.temp_lag3", "air.temp_lag2",
                      "air.temp_lag1", "air.temp",
                      "public_tt", "pop", "build_up",
...
save(save_list, file = "~/Desktop/SAE_with_more_lags/medfever_stack_2014_norm_w_dates.RData")

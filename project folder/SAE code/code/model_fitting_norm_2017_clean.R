...
med_2014 <- read.csv("~/OneDrive/Desktop/cluster_medfever_14.csv", stringsAsFactors = FALSE)[,-1]
med_2014$year <- 2014
med_2014$row_num <- 1:nrow(med_2014)



med_2017 <- read.csv("~/OneDrive/Desktop/cluster_medfever_17.csv", stringsAsFactors = FALSE)[,-1]
med_2017$year <- 2017
...
ground_truth_R <- data.frame(Region = str_replace(burkina_shape_Region$NAME_1, "-", " "),
                             tru_val = c(0.609207678, 0.84334736, 0.598458806, 0.806970575,
                                         0.727448959, 0.695179117, 0.808722575, 0.771775153,
                                         0.712514362, 0.820978152, 0.838615637, 0.671700711,
                                         0.804862133))
...
med_2017 <- read.csv("~/OneDrive/Desktop/medfever_DS_2017.csv", stringsAsFactors = FALSE)[,-1]
med_2017 <- med_2014[c(1:29, 31:35, 69, 36:43, 70, 44:64, 30, 65:68),]
...
pop_weighted_DS <- function(X)
{
    ...
    r_spdf_higher_res_int <- raster::intersect(r_spdf_higher_res, pop_2017)
    r_spdf_higher_res_int_clip <- mask(x = r_spdf_higher_res_int, mask = pop_2017)
    
    DS_extracted_fits <- raster::extract(r_spdf_higher_res_int_clip, DS_shape_W)
    DS_extracted_pop <- raster::extract(pop_2017, DS_shape_W)
...
load("~/Desktop/SAE_with_more_lags/medfever_stack_2017_norm_w_dates.RData")
...
# pp.res_precip_lag1_2 <- inla(y ~ 0 + b0 + precip_lag1_grouped_q7 +
#                                  f(spatial.field, model = medfever.spde),
#                              family = "binomial", Ntrials = total,
#                              data = inla.stack.data(join.stack),
#                              control.predictor = list(A = inla.stack.A(join.stack),
...
tmp_slcpo <- c("precip_lag1" = slcpo(pp.res_precip_lag1),
               "precip_lag1_grouped" = slcpo(pp.res_precip_lag1_2),
               "air.temp_lag1" = slcpo(pp.res_a.t_lag1),
               "air.temp_lag1_grouped" = slcpo(pp.res_a.t_lag1_2))


tmp_err <- t(data.frame(precip_lag1 = get_MAE(pp.res_precip_lag1$summary.fitted.values[idx, "mean"]),
                        precip_lag1_grouped = get_MAE(pp.res_precip_lag1_2$summary.fitted.values[idx, "mean"]),
                        air.temp_lag1 = get_MAE(pp.res_a.t_lag1$summary.fitted.values[idx, "mean"]),
                        air.temp_lag1_grouped = get_MAE(pp.res_a.t_lag1_2$summary.fitted.values[idx, "mean"]),
                        row.names = c("Region err", "Cluster err")))

cbind("slcpo" = tmp_slcpo, tmp_err)

summary(pp.res_precip_lag1)$dic$dic
summary(pp.res_precip_lag1)$waic$waic
summary(pp.res_precip_lag1)$mlik[2]

summary(pp.res_precip_lag1_2)$dic$dic
summary(pp.res_precip_lag1_2)$waic$waic
summary(pp.res_precip_lag1_2)$mlik[2]

summary(pp.res_a.t_lag1)$dic$dic
summary(pp.res_a.t_lag1)$waic$waic
summary(pp.res_a.t_lag1)$mlik[2]

summary(pp.res_a.t_lag1_2)$dic$dic
summary(pp.res_a.t_lag1_2)$waic$waic
summary(pp.res_a.t_lag1_2)$mlik[2]


round(pp.res_precip_lag1$summary.fixed[,c(1,3,5)], 3) %>% apply(1, function(X) { paste(X[1], " [", X[2], ", ", X[3], "]", sep = "") }) %>% as.data.frame()
round(pp.res_precip_lag1_2$summary.fixed[,c(1,3,5)], 3) %>% apply(1, function(X) { paste(X[1], " [", X[2], ", ", X[3], "]", sep = "") }) %>% as.data.frame()

round(pp.res_a.t_lag1$summary.fixed[,c(1,3,5)], 3) %>% apply(1, function(X) { paste(X[1], " [", X[2], ", ", X[3], "]", sep = "") }) %>% as.data.frame()
round(pp.res_a.t_lag1_2$summary.fixed[,c(1,3,5)], 3) %>% apply(1, function(X) { paste(X[1], " [", X[2], ", ", X[3], "]", sep = "") }) %>% as.data.frame()


#####################################################################################################


pp.res_NDVI <- inla(y ~ 0 + b0 + NDVI +
                      f(spatial.field, model = medfever.spde), 
                  family = "binomial", Ntrials = total,
                  data = inla.stack.data(join.stack),
                  control.predictor = list(A = inla.stack.A(join.stack),
...
pp.res_tt_grouped <- inla(y ~ 0 + b0 + TT_grouped_q6 +
                              f(spatial.field, model = medfever.spde),
                          family = "binomial", Ntrials = total,
                          data = inla.stack.data(join.stack),
                          control.predictor = list(A = inla.stack.A(join.stack),
...
tmp_slcpo <- c("pp.res_tt" = slcpo(pp.res_tt),
               "pp.res_tt_grouped" = slcpo(pp.res_tt_grouped),
               "pp.res_0" = slcpo(pp.res_0))


tmp_err <- t(data.frame(pp.res_tt = get_MAE(pp.res_tt$summary.fitted.values[idx, "mean"]),
                        pp.res_tt_grouped = get_MAE(pp.res_tt_grouped$summary.fitted.values[idx, "mean"]),
                        pp.res_0 = get_MAE(pp.res_0$summary.fitted.values[idx, "mean"]),
                        row.names = c("Region err", "Cluster err")))

cbind("slcpo" = tmp_slcpo, tmp_err)

summary(pp.res_tt)$dic$dic
summary(pp.res_tt)$waic$waic
summary(pp.res_tt)$mlik[2]

summary(pp.res_tt_grouped)$dic$dic
summary(pp.res_tt_grouped)$waic$waic
summary(pp.res_tt_grouped)$mlik[2]

summary(pp.res_0)$dic$dic
summary(pp.res_0)$waic$waic
summary(pp.res_0)$mlik[2]


round(pp.res_tt$summary.fixed[,c(1,3,5)], 3) %>% apply(1, function(X) { paste(X[1], " [", X[2], ", ", X[3], "]", sep = "") }) %>% as.data.frame()
round(pp.res_tt_grouped$summary.fixed[,c(1,3,5)], 3) %>% apply(1, function(X) { paste(X[1], " [", X[2], ", ", X[3], "]", sep = "") }) %>% as.data.frame()
round(pp.res_0$summary.fixed[,c(1,3,5)], 3) %>% apply(1, function(X) { paste(X[1], " [", X[2], ", ", X[3], "]", sep = "") }) %>% as.data.frame()


##################################################################################


pp.res_0_with_D_RE <- inla(y ~ 0 + b0 +
...
pp.res_1_with_D_RE <- inla(y ~ 0 + b0 + precip_lag1_grouped_q7 +
                               f(District, model = "iid") +
                               f(spatial.field, model = medfever.spde),
                           family = "binomial", Ntrials = total,
                           data = inla.stack.data(join.stack),
...
tmp_slcpo <- c("pp.res_0_with_D_RE" = slcpo(pp.res_0_with_D_RE),
               "pp.res_1_with_D_RE" = slcpo(pp.res_1_with_D_RE))


tmp_err <- t(data.frame(pp.res_0_with_D_RE = get_MAE(pp.res_0_with_D_RE$summary.fitted.values[idx, "mean"]),
                        pp.res_1_with_D_RE = get_MAE(pp.res_1_with_D_RE$summary.fitted.values[idx, "mean"]),
                        row.names = c("Region err", "Cluster err")))

cbind("slcpo" = tmp_slcpo, tmp_err)

summary(pp.res_0_with_D_RE)$dic$dic
summary(pp.res_0_with_D_RE)$waic$waic
summary(pp.res_0_with_D_RE)$mlik[2]

summary(pp.res_1_with_D_RE)$dic$dic
summary(pp.res_1_with_D_RE)$waic$waic
summary(pp.res_1_with_D_RE)$mlik[2]

round(pp.res_0_with_D_RE$summary.fixed[,c(1,3,5)], 3) %>% apply(1, function(X) { paste(X[1], " [", X[2], ", ", X[3], "]", sep = "") }) %>% as.data.frame()
round(pp.res_1_with_D_RE$summary.fixed[,c(1,3,5)], 3) %>% apply(1, function(X) { paste(X[1], " [", X[2], ", ", X[3], "]", sep = "") }) %>% as.data.frame()

round(summary(pp.res_0_with_D_RE)$hyperpar[,c(1,3,5)], 3) %>% apply(1, function(X) { paste(X[1], " [", X[2], ", ", X[3], "]", sep = "") }) %>% as.data.frame()
round(summary(pp.res_1_with_D_RE)$hyperpar[,c(1,3,5)], 3) %>% apply(1, function(X) { paste(X[1], " [", X[2], ", ", X[3], "]", sep = "") }) %>% as.data.frame()
...
load(file = "~/OneDrive/Desktop/MIS_cluster_pts_2017.RData")

names(pt_m)[c(23,24)] <- c("u5_size", "med_fever")

# pt_m17 <- pt_m[-which(pt_m$LATNUM == 0),]
pt_m17 <- pt_m
...
ggplot() + geom_sf(data = DS_merge,
                   aes(fill = rate)) +
    scale_fill_stepsn(limits = c(.5,1),
...
saveRDS(spdf, file="~/Desktop/SAE_with_more_lags/2017_1x1km_spdf.rds")




X <- pp.res_1_with_D_RE$summary.fitted.values[idx, "mean"]
DS_INLA_fits <- DS_pop_weighted_fits[,c("Admin2", "rate")]
DS_INLA_fits <- as.data.frame(cbind(DS_INLA_fits[,1], rep(2017, nrow(DS_INLA_fits)),
                                    DS_INLA_fits[,2]))
names(DS_INLA_fits) <- c("NOMDEP", "year", "saep.est")


X <- pp.res_1_with_D_RE$summary.fitted.values[idx, "0.025quant"]
DS_INLA_fits <- cbind(DS_INLA_fits, DS_pop_weighted_fits[,"rate"])
names(DS_INLA_fits) <- c("NOMDEP", "year", "saep.est", "ci_l")

X <- pp.res_1_with_D_RE$summary.fitted.values[idx, "0.975quant"]
DS_INLA_fits <- cbind(DS_INLA_fits, DS_pop_weighted_fits[,"rate"])
names(DS_INLA_fits) <- c("NOMDEP", "year", "saep.est", "ci_l", "ci_u")


write.csv(DS_INLA_fits, "~/OneDrive/Desktop/medfever_DS_2017_INLA_fitted_1x1km.csv")
